...
### 6.2.1 M2 合体（A 线真实数据 → B 线网页，两个终端）

```bash
python backend/api.py --no-mock        # 终端 1：只广播 ingest 进来的真实帧（浏览器开 http://127.0.0.1:8000/ ）
python backend/run_pipeline.py --source synthetic --pattern blink --seconds 30 --stub \
    --json metrics/logs/last.json --jsonl metrics/logs/stream.jsonl --post auto   # 终端 2
python metrics/scripts/check_a_line_p5_m2.py   # 一条命令验证整条链路（17 项，真起两个进程）
```
...
| **`git log` 里提交标题前多出一个看不见的字符**（首字符码点 `0xFEFF`） | **PowerShell 5.1 的 `Out-File -Encoding utf8` / `Set-Content -Encoding UTF8` 写出的文件带 BOM**，再 `git commit -F 那个文件` 就把 BOM 带进了标题。⚠️ 注意与上一行**要求正好相反**：`.ps1` 必须带 BOM，**喂给 `git commit -F` 的消息文件必须无 BOM** | 消息文件用编辑器/本工具的 write 存成 **UTF-8 无 BOM**；或改用 `git commit -m`。**写完先验一眼**：`git log -1 --pretty=%s`，必要时 `git commit --amend -F 新文件` 修回来 |
| **编辑器/静态检查报 `无法解析导入 "csi" / "sensor" / "omv" / "machine" / "pyb"`（`reportMissingImports`）**，通常出现在 `board/openmv/*.py` | 这几个模块**只存在于相机的 MicroPython 固件里**，PC 的 CPython 里根本没有它们 —— **不是缺依赖**，是这类文件本来就只能在相机上跑 | ⛔ **不要 `pip install`**（`csi` / `sensor` / `omv` 这些包不存在，装了也没用）；⛔ **也不要为了消除告警去删 import**（删了相机上直接跑不起来）。已按仓库既有做法给这几行加 `# type: ignore`（与 `host_capture_test.py` 对 `cv2` / `serial` 的处理一致）。要在本机验这类文件，用 `board/openmv/offline_check.py`（注入假模块跑真脚本） |
