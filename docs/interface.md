...
| 12 | **A 线** | **全局 45 fps 切换**（v1.1）：§0 帧率 30→45、§3.5 FIR 采样率 30→45；系数与黄金参考已重生成，csynth/cosim 已重跑 | 确认采集/关键点/指标链路能跑 45 fps，rPPG q 序列按 45 Hz 生成并与新黄金参考对拍 | 🟡 **2026-09-15 A 线：链路侧与时间序列侧已核，rPPG 端到端待 P3**（见 5.4） |
...
> 本文件**不代签**：A 线只登记自己的核实结论，群公告与三方确认由人类完成。
