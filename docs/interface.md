...
> **状态：草案 v0.95 —— 待 A / B / C 三方会签后冻结为 v1.0。**
...
11. [ ] **A 线（新增，P0-1）**：4.6 节的 **Q1.15 量化口径**与"**`count==0` 的帧一律丢弃**"是否认可？
    > 建议做法：直接跑 `python fpga/sim/q15_ref.py --selftest`（应 PASS），再用你们自己的
    > `--sums-csv` 跑一遍真实数据，把得到的 q 序列与你们的实现逐样本对拍 —— 照 4.4 节的范式，
    > **用代码确认比口头认可硬**。若你们希望量化口径不同（例如留 2 倍余量、或先做去均值），
    > 请在群公告里给出理由，按 §7 流程改本契约，**不要**两边各实现一套。
