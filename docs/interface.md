...
> **第 5.2 节的会签状态：B 线第 3 项已于 2026-09-13 补签（见 5.2.1）；A 线的 6 项尚未收到书面表态 —— 本文件不代签、不假设"无异议"**，
> 请 A 线按第 5.3 节的流程表态（认可即补签；有异议按第 7 节提变更，走完出 v1.2）。
...
| 12 | **A 线** | **全局 45 fps 切换**（v1.1）：§0 帧率 30→45、§3.5 FIR 采样率 30→45；系数与黄金参考已重生成，csynth/cosim 已重跑 | 确认采集/关键点/指标链路能跑 45 fps，rPPG q 序列按 45 Hz 生成并与新黄金参考对拍 | ⏳ 待 A 线表态 |

> ⚠️ **本表的状态是事实描述**：截至 2026-09-13，**第 3 项（B 线）已补签**；
> **第 1、2、4、7、11、12 项（均属 A 线）尚未收到书面表态 —— 本文件不代签、不假设"无异议"**。
