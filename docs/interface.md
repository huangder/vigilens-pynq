...
> **状态：🔒 v1.1（2026-09-16 三方会签完成）。**
> 沿革：v1.0 于 2026-09-11 冻结 → v1.1（"全局 45 fps"，C 线 09-13 发起）已于 09-16 会签完成。
> 按 `AGENTS.md` 第 4 节第 5 步「会签升级版本号」，`backend/contract.py` 的 `CONTRACT_VERSION`
> 已同步为 **`v1.1`**；第 1/2 节 A→B schema 未变，`frontend/mock.js` 无需改动。
> 第 0~4 节口径已定稿，C 线侧每条均有实测证据（见第 5.1 节与 `fpga/report/`）；
> **第 5.2 节的会签状态：✅ 三方会签完成（2026-09-16）**——B 线第 3 项 09-13 补签（见 5.2.1）、
> A 线 6 项 09-15 补签（见 5.4），A 线 09-16 按 §5.3 在群里公告，**B/C 已回复"收到、不冲突"**。
> 其中第 7、12 项按契约明文**待真实视频**后回填（不阻塞冻结）。
> 本文件**不代签、不假设"无异议"**：上条记录的是 B/C 在群里的实际回复。
> v1.1 变更：§0 帧率 30→45 fps、§3.5 FIR 采样率 30→45 Hz（系数与黄金参考已重生成；csynth/cosim 已重跑，见第 6 节）。
>
> 🚧 **v1.2 草案（2026-09-20，C 线发起，尚未会签）：§0 帧率 45→30 fps、§3.5 FIR 采样率 45→30 Hz。**
> 目的：前期适配低帧率采集源（OpenMV Cam H7 在 640×480 上实测只有 10~20 fps）。
> 系数与黄金参考已按 30 Hz 重生成并逐样本对拍通过（见第 6 节 2026-09-20 行），
...
| 帧率 | **30 fps**（🚧 v1.2 草案；v1.1 曾为 45 fps。仅影响时间序列，IP 本身不感知帧率） | 640×480 RGB888@30fps |
...
| 采样率 | **30 Hz**（与图像 30 fps 同频；🚧 v1.2 草案，v1.1 曾为 45 Hz） |
...
| 实测频率响应 | −3 dB 点 0.895 / 3.305 Hz；1.0~3.0 Hz 起伏 1.90 dB；0.3 Hz −18.3 dB；8 Hz −80.2 dB |
...
| 资源 / 时序（实测） | LUT 4043 / FF 6174 / **BRAM 0** / DSP 26；**Final II = 1**；Estimated Fmax **154.38 MHz**（**@45 fps，2026-09-13 实测** —— v1.1 时代的数字）。⚠️ **@30 fps 未重跑 csynth/cosim**：结构/阶数/位宽未变、只有 63 个系数值不同（HLS 会把系数常量折叠），预计资源与时序相同，但**未验证**，不得当结论引用 |
...
>
> 沿革：v1.1（45 fps）时过渡带为 0.375 Hz、需 N ≳ 158 / 236 —— **降到 30 Hz 后这条限制略有缓解**
> （0.375 → 0.250 Hz），但**仍然做不了** 0.1~0.5 Hz 呼吸带，结论不变。
...
## 5. 会签状态（v1.0 冻结 → ✅ v1.1 会签完成，2026-09-16）

### 5.1 已核销项（C 线自行闭环，均附实测证据）

| # | 事项 | 结论与证据 |
|---|---|---|
| 5 | `motion_thresh` 默认 16 | ✅ 2026-09-10 定稿；⚠️ **A 线若按 OpenCV 口径复核后要改，必须重生成黄金参考**（改则走第 7 节）。**2026-09-16 闭环**：三方拍板**保留 16**，A 线把自己的实现（灰度式 + 分辨率 + 阈值）统一到该口径，A↔C 运动量对拍 **9/9 逐项全等（容差 0）**；**C 线黄金参考无需重生成**。细节见 §5.4 末节 |
...
|---|---|---|---|---|
| 1 | **A 线** | `frames.bin` 的 **RGB 通道顺序**与**半开区间 ROI** 是否认可？（逐点相等的唯一前提） | 用你们自己的 NumPy 代码对 `fpga/sim/data/frames.bin` 复算一遍 | ✅ **A 线 09-15 补签：认可；三方会签完成 09-16**（证据见 5.4） |
| 2 | **A 线** | `roi_statistic` 的整型累加是否就是 rPPG / 质量评分要用的那份时间序列口径？ | 同上：与你们自己那套实现对拍 | ✅ **A 线 09-15 补签：认可；三方会签完成 09-16**（证据见 5.4） |
| 3 | **B 线** | 第 1 节 JSON schema 与第 2 节 6 值枚举是否已全部支持？ | 跑 `node frontend/mock.js --selftest`（应 `ok:true`） | ✅ **B 线 09-13 补签：认可；三方会签完成 09-16**（证据见 5.2.1） |
| 4 | **A 线** | 按 **4.4 节对拍脚本**验证灰度口径一致（`assert` 必须通过） | 直接跑该节脚本 | ✅ **A 线 09-15 补签：认可；三方会签完成 09-16**（证据见 5.4） |
...
| 11 | **A 线** | 4.6 节的 **Q1.15 量化口径**与"**`count==0` 的帧一律丢弃**"是否认可？ | 跑 `python fpga/sim/q15_ref.py --selftest`，再用 `--sums-csv` 与自己实现对拍 | ✅ **A 线 09-15 补签：认可；三方会签完成 09-16**（证据见 5.4） |
...
>
> ✅ **2026-09-16：三方会签完成。** A 线按 §5.3 在群里公告了本节结论（逐条结论 + `motion_thresh`
> 差异表，内容见 5.4），**B/C 均已回复"收到、不冲突"** —— 故第 **1、2、3、4、11** 项会签完成。
> 第 **7**、**12** 项按契约明文**待真实视频**后回填（**不阻塞冻结**，见各自的状态说明）。
> ⚠️ 本文件仍**不代签**：以上记录的是 B/C 在群里的实际回复；后续如有异议，按第 7 节流程提变更。
...
> ✅ **2026-09-16 三方拍板：采纳上述建议 —— 统一到 C 线那套（16 / 384×288 / 冻结式）。**
> A 线随即完成对齐（**本节改动的是 A 线实现与 `config.yaml`，未动本文件第 0~4 节任何口径**）：
>
> | 项 | 改前（A 线） | 改后（A 线） |
> |---|---|---|
> | `motion_thresh_gray` | 25 | **16** |
> | 灰度式 | `cv2.cvtColor` | **冻结式 `(77R+150G+29B+128)>>8`**（按 BGR 的 R=2/G=1/B=0 取值） |
> | 计算分辨率 | 640×480 | **384×288**（3/5 相位点采样） |
>
> **验收（本机真实运行，容差 0）**：`python metrics/scripts/check_motion_golden.py`
> → 灰度逐像素**不一致 0 个像素**（尺寸 A 线 288×384 = 黄金参考）；运动量 **9/9 帧对逐项全等**。
> 证据：`metrics/evidence/2026-09-16_a_line_p1_golden_checks.json`（重新归档）。
> **未重生成 C 线黄金参考**（因为取的是 16，C 线无需重跑 csim/csynth/cosim）。
> 另：契约 §5.2 第 5 项的具体**数值**仍属"占位待标定"，按拍板结论留到 **P4** 用真实视频定稿。
>
> ⚠️ **本节只登记核实结论与拍板结论，未改动本文件第 0~4 节的任何口径。**
> 群公告与三方确认仍由人类完成；本表状态如有异议，按第 7 节流程提出。
>
> ✅ **2026-09-16 补充：本节结论已在群里公告**（逐条结论 + 上表的实测差异），
> **B/C 均已回复"收到、不冲突"** —— 第 1/2/4/11 项会签完成（见 §5.2）。
> 第 7 项与第 12 项中"rPPG 端到端"的部分按契约明文**待真实视频**后回填。
