...
//      N = FIR_NUM_TAPS = 63，群延迟 = (N-1)/2 = 31 个样本（@30 fps = 1033 ms；🚧 v1.2 草案，v1.1 曾为 @45 fps = 689 ms）
//
//  【四条必须与 A 线逐字一致的运算约定】
...
//
//  【呼吸带说明】本 IP v1 只做心率带（0.7~3.5 Hz @30 fps；🚧 v1.2 草案）。同一阶数（63）
//    在 30 fps 下也**做不了** 0.1~0.5 Hz 的呼吸带（过渡带 0.250 Hz 与整个通带 0.4 Hz 同量级；
//    v1.1 的 @45 fps 时是 0.375 Hz），呼吸带需 PS 侧降采样后再用同名 IP 的另一组系数 —— 见契约 3.5 的已知限制。
// =============================================================================
