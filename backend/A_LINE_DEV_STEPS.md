...
### P1 —— A ↔ C 黄金参考对拍（任务号：A10 的前置；对应契约 §5.2 第 1/2/4/11 项，并覆盖第 12 项的 rPPG 部分）
...
### P3 —— rPPG 链路（任务号：A 线增强项，对应契约 §3.6/§4.6 的时间序列链路）
...
```powershell
. .\env.ps1                                              # 载入本机工具链
.venv\Scripts\python.exe -m pytest -q                    # 期望 49 passed（只增不减）
...
> ✅ P0 已完成（2026-09-15）。
> **下一步 = P1.0 → P1.1**：重生成输入向量（**注意用 45 Hz 版黄金参考**），跑通灰度口径对拍。
> 跑完就有第一份**带真实数字**的结论，可以拿去群里做契约表态。
